...
export default function LandingPage() {
  ...
  const visibleUpdates = useMemo(
    () => updates.filter((item) => item.is_visible),
    [updates],
  );
    ...
    async function fetchContentUpdates() {
      ...
      try {
        const [newsRes, achievementsRes] = await Promise.all([
          fetch(apiUrl(API_ENDPOINTS.contentNews), {
            signal: controller.signal,
          }),
          fetch(apiUrl(API_ENDPOINTS.contentAchievements), {
            signal: controller.signal,
          }),
        ]);
        ...
        const [newsJson, achievementsJson] = await Promise.all([
          newsRes.json(),
        ...
        const mappedAchievements = (achievementsJson.results || []).map(
          (item) => ({
            id: item.id,
            model: "achievement",
  ...
  useEffect(() => {
    ...
    async function fetchPowerSources() {
      ...
      try {
        const response = await fetch(apiUrl(API_ENDPOINTS.powerSources), {
          signal: controller.signal,
        });
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    async function fetchIndustries() {
      setIndustriesError("");
      try {
        const response = await fetch(apiUrl(API_ENDPOINTS.industries), {
          signal: controller.signal,
        });
        if (!response.ok) {
        ...
        if (isMounted) {
          setIndustries(
            (json.results || []).filter((item) => item.is_visible !== false),
          );
        }
      } catch (error) {
        if (error.name !== "AbortError" && isMounted) {
      ...
      <main className="bg-steel-50">
            ...
            <div className="relative mx-auto flex h-full max-w-7xl items-center px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
              <div className="w-[92vw] max-w-136 rounded-xl border border-white/20 bg-steel-950/42 p-4 shadow-[0_18px_48px_rgba(0,0,0,0.25)] backdrop-blur-md sm:rounded-2xl sm:p-6">
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white/70 sm:text-xs">
                  Custom Valve Automation for Critical Process Industries
                </p>

                <h1 className="mt-3 max-w-[12ch] text-[clamp(2.1rem,5vw,4rem)] font-extrabold leading-[0.9] tracking-tight text-white">
                  <span className="text-brand-500">Reliable</span>
                  <span className="block">Actuation.</span>
                ...
                <p className="mt-3 max-w-[34ch] text-xs leading-relaxed text-white/75 sm:text-sm">
                  High-performance valve automation solutions designed for
                  safety, precision, and demanding industrial applications.
                </p>
              ...
              >
                ...
                >
                  {slides.map((slide, idx) => (
                    <div
                      key={idx}
                      className="grid w-full shrink-0 gap-2 sm:gap-3 md:grid-cols-3"
                    >
                      {slide.map((item) => (
                        <article
          ...
          <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-11">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <p className="max-w-xl text-sm leading-relaxed text-steel-600 sm:text-right">
                Explore our range of precision-engineered valve actuators and
                automation systems, built to deliver reliable control across
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {powerSources.map((source) => (
                <Link
                  ...
                  href={
                    source.slug
                      ? `/products/${encodeURIComponent(source.slug)}`
                      : "/products"
                  }
        ...
        <section className="bg-steel-50">
          <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
            ...
            {industriesError && (
              <p className="mt-3 text-sm text-red-600">{industriesError}</p>
            )}
            ...
            >
              <div
                ...
                style={{
                  animationPlayState: isIndustryPaused ? "paused" : "running",
                }}
              >
                {industryTrackItems.map((industry, index) => (
                  <Link
                    ...
                    href={
                      industry.slug
                        ? `/products?industries=${encodeURIComponent(industry.slug)}`
                        : "/products"
                    }
        ...
        <section className="bg-steel-100">
          <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:pt-8 lg:pb-12">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-700"></p>
            <h2 className="mt-1 text-4xl font-extrabold leading-[0.95] tracking-tight text-steel-900 sm:text-5xl">
              Our
              <br />
              ...
              {partnerRows.map((row, rowIndex) => {
                ...
                return (
                  <div key={rowIndex} className="overflow-hidden">
                    <div
                      className={`${trackClass} flex w-max items-center gap-8 sm:gap-12`}
                    >
